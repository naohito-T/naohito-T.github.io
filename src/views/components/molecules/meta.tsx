...
export const Meta: NextComponentType<NextPageContext, null, Props> = ({
  ...
  const title = pageTitle ?? C.PAGE_TITLE;
  const description = pageDesc ?? C.PAGE_DESCRIPTION;
  const type = pageAsPath === '/' ? 'blog' : 'article';
  const url = pageFullPath ?? C.PAGE_URL;
  const siteName = C.PAGE_TITLE;
  const ogImg = ogpImage ?? C.PAGE_OG_IMAGE;
  return (
    <Head>
      <title>{title}</title>
